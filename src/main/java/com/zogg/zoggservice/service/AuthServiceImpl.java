...
public class AuthServiceImpl implements AuthService {
  ...
  public TokenResponse verifyOtp(@Valid VerifyOtpRequest request) {
    ...
    String storedOtp = redisService.get(CommonUtils.getRedisKeyForOtp(request.getPhoneNumber()))
        .orElseThrow(() -> CommonUtils.logAndGetException("OTP expired or not found."));
  ...
  private TokenResponse generateTokenResponse(User user) {
    ...
    return TokenResponse.builder().user(UserMapper.INSTANCE.toDto(user)).token(token).tokenType("Bearer").expiresIn(999998).build();
  }
  ...
  private Long sendOtpToUser(String phoneNumber) {
    ...
    redisService.setWithExpiry(CommonUtils.getRedisKeyForOtp(phoneNumber), String.valueOf(otp),
        OTP_EXPIRY_SECONDS);
