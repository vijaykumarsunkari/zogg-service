...
public class VoucherServiceImpl implements VoucherService {
  ...
  public Object addVoucher(VoucherDto voucherDto) {
    BrandCollection brandCollection = brandCollectionRepository.findById(voucherDto.getBrandId())
        .orElse(null);
  ...
  public List<VoucherDto> getVouchers(String userId) {
    ...
    List<String> redeemedVoucherIds = couponCollectionRepository.findAllByUserId(
        Integer.parseInt(userId)).stream().map(CouponCollection::getVoucherId).toList();

    List<VoucherCollection> availableVouchers = voucherCollectionRepository.findAllByIdNotIn(
        redeemedVoucherIds);

    List<VoucherDto> voucherDtos = VoucherMapper.INSTANCE.toDto(availableVouchers);

    Set<String> brandIds = voucherDtos.stream().map(VoucherDto::getBrandId)
        .collect(Collectors.toSet());

    Map<String, BrandCollection> brandIdToBrandMap = brandCollectionRepository.findAllById(brandIds)
        .stream().collect(Collectors.toMap(BrandCollection::getId, brand -> brand));

    voucherDtos.forEach(voucher -> {
